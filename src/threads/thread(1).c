...
{
  int temp;
  temp = CONVERT_TO_INT_NEAR(100 * load_avg);
  return temp;
}
...
thread_increment_recent_cpu(void)
{
  ...
  current->recent_cpu = INT_ADD(current->recent_cpu, 1);
}
...
thread_calculate_load_avg(void)
{
  ...
  load_avg = FP_MUL (CONVERT_TO_FP (59)/ 60, load_avg) + FP_DIV(CONVERT_TO_FP(ready_threads), 60);
...
thread_calculate_priority(struct thread *t)
{
  ...
  t -> priority =  CONVERT_TO_INT_ZERO(INT_SUB(CONVERT_TO_FP(PRI_MAX) - (t->recent_cpu / 4), 2 * t->nice));
